-- here
--[[
	// Dont Copy this Script. Please!

	// Written by: DakPlayer75
]]
...
local CoreGuiService = game:GetService('CoreGui') --game:GetService("CoreGui")
...
local GuiRoot = CoreGuiService:WaitForChild('NewRobloxGui')
local TenFootInterface = require(GuiRoot.Modules.TenFootInterface)
local isTenFootInterface = game.Lighting.TenFootInterface.Value
...
local function CreateSettingsIcon(topBarInstance)
	local MenuModule = require(game:GetService("CoreGui").NewRobloxGui.Modules.Settings.SettingsHub)
	...
	require(game.ReplicatedStorage:WaitForChild("GlobalEvents")).EscapeKeyPressed:connect(function()
		toggleSettings()
	end)
...
local function Create3DSettingsIcon(topBarInstance, panel, menubar)
	local MenuModule = require(game.CoreGui.NewRobloxGui.Modules.Settings.SettingsHub)
...
local function CreateChatIcon()
	local chatEnabled = true --game:GetService("UserInputService"):GetPlatform() ~= Enum.Platform.XBoxOne
